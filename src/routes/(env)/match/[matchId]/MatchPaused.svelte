...
<div class="absolute top-0 left-0 w-full h-full bg-blue-950/30 backdrop-blur p-[15px]">
	...
	<TopbarLayout>
		<SpecialButton slot="topbar-left" preload={false} href="/menu/main">Menu</SpecialButton>
		<svelte:fragment slot="topbar-right">
			<div class="flex flex-row gap-[5px]">
				<SpecialButton on:click={restart}>restart</SpecialButton>
				<SpecialButton forceFocusOnMount on:click={proceed}>Resume</SpecialButton>
			</div>
		</svelte:fragment>
		...
		<div class="flex flex-col gap-[15px] items-start">
			...
			<Card class="flex flex-col gap-[10px] w-max">
				...
				>
					...
					<div class="text-right">Pause</div>
				</div>
			</Card>
		</div>
	</TopbarLayout>
